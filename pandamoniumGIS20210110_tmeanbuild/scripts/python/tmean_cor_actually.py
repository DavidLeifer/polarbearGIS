...
cwd = os.getcwd()
#read in tmean_bil2tif_resize as list of file directories
tmean_bil2tif_resize = cwd + '/data/tmean_bil2tif_resize/'
tmean_bil2tif_resize_list = glob(os.path.join(tmean_bil2tif_resize, '*.tif'))
tmean_bil2tif_resize_list.sort()
#print(tmean_bil2tif_resize_list)

#read in index files as list of file directories
tmean_pearson_output = cwd + '/data/tmean_pearson_output/'
tmean_pearson_output_list = glob(os.path.join(tmean_pearson_output, '*.tif'))
tmean_pearson_output_list.sort()
#print(tmean_pearson_output_list)

for example_tif, index_tif in zip(tmean_bil2tif_resize_list,tmean_pearson_output_list):
    base = os.path.basename(example_tif)
    year = base[21:25]
    base2 = os.path.basename(index_tif)
    #load in precipitation raster tif, output from tmean_cor.py located in /data/tmean_bil2tif_resize/
    width = 1405
    height = 621
    with rasterio.open(example_tif, mode="r+") as src:
        tmean_raster = src.read(1, out_shape=(1, int(height), int(width)))
    #load in 1981 index mask raster tif, output from tmean_cor.py located in /data/tmean_pearson_output/
    width = 1405
    height = 621
    ...
    for x, i in zip(tmean_raster, index_raster): 
    ...
    #array scaled by 1,000,000,000,000
    scaled_array = np.multiply(npa, 1000000000000)
    ...
    data_dir = "/data/tmean_pearson_final/tmean_pearson_final_"
    dst_filename = cwd + data_dir + year + ".tif"

    with rasterio.open(dst_filename, 'w', **naip_meta) as dst:
        dst.write(newarr2, 1)

print("Finished tmean_cor_actually.py")
